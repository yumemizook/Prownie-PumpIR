...
addtoLB.addEventListener("click", async () => {
  ...
  try {
    ...
    // Collect all user scores, deduplicate by songKey, lvl, rate, and score
...
>>>>>>> parent of f1fa738 (.)
      ...
      if (!scoreKeySet.has(uniqueKey)) {
        scoresToAdd.push({ scoreData, songKey, lvl, rate, score, uniqueKey });
        scoreKeySet.add(uniqueKey);
      }
    }

    // Group scores by songKey for batch processing
    const groupedBySong = {};
    ...
    for (const songKey in groupedBySong) {
      ...
      // Build a set of existing (lvl, rate, score) for this user/song
